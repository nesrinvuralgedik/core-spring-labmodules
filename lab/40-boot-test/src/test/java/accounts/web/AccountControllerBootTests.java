...
public class AccountControllerBootTests {
	...
	public void getAllAccounts() throws Exception{
		...
		for (int i = 1; i <= 21; i++){
			allAccounts.add(new Account("Number" + i, "Account" + i));
		}

		given(accountManager.getAllAccounts()).willReturn(allAccounts);

		mockMvc.perform(get("/accounts"))
				.andExpect(status().isOk())
				.andExpect(content().contentType(MediaType.APPLICATION_JSON))
				.andExpect(jsonPath("$.length()").value(allAccounts.size()))
				.andExpect(jsonPath("$[0].name").value("Account1"))
				.andExpect(jsonPath("$[0].number").value("Number1"))
				.andExpect(jsonPath("$[20].name").value("Account21"))
				.andExpect(jsonPath("$[20].number").value("Number21"));
